...
from .models import Product, Contact, icon, SpecificBanner
...
def index(request):
    ...
    params = {'allprods':all_products, 'category_list':category_list, 'icons':icons}
...
def viewall(request, my_category):
    all_products = Product.objects.all();
    specific_banners = SpecificBanner.objects.all();
    specific_banners = SpecificBanner.objects.filter(sb_category = my_category);
    category_list = []
    for i in all_products:
        if (i.product_category not in category_list):
            category_list.append(i.product_category)
    ...
    params = {'product_cat' : product_cat, 'temp':temp, 'allprods': all_products, 'category_list': category_list, 'specific_banners':specific_banners}
